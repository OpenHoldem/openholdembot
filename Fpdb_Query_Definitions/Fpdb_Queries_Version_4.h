...
// GENERAL STATS
// ICON disabled, as not supported by FPDB 4.
...
// PREFLOP STATS
#define FPDB_QUERY_SUPPORT__STEAL_ATTEMFPDB				  (TRUE)	// "steal_attemfpdb"
...
// FLOP Other stats
#define FPDB_QUERY_SUPPORT__FLOP_FOLD_TO_3B			(FALSE)	// "flop_fold_to_3bet"   // 3b is a reraise, we should have a "flop_fold_to_raise" stat first.
...
// TURN Other stats
#define FPDB_QUERY_SUPPORT__TURN_FOLD_TO_3B			(FALSE)	// "turn_fold_to_3bet" // 3b is a reraise, we should have a "turn_fold_to_raise" stat first.
...
// RIVER Other stats
#define FPDB_QUERY_SUPPORT__RIVER_FOLD_TO_3B			(FALSE)	// "river_fold_to_3bet" // 3b is a reraise, we should have a "river_fold_to_raise" stat first.
...
//PREFLOP STATS
+ (FPDB_QUERY_SUPPORT__STEAL_ATTEMFPDB ? 1 : 0)
...
t_QueryDefinition query_definitions[k_number_of_fpdb_stats] =
{
	...
	// FPDB 
	{
		...
		"SELECT	sum(street0VPI)*1.0/sum(street0VPIChance)*1.0 as result \
		 FROM	HandsPlayers, HandsActions, Hands, Actions, Players, Gametypes \
		 WHERE	HandsPlayers.playerId = Players.id AND \
				HandsPlayers.handId = Hands.id AND \
				HandsActions.handId = Hands.id AND \
				HandsActions.actionNo = Actions.id AND \
				(Actions.id = 7 OR Actions.id = 8) AND \
				Hands.gametypeId = Gametypes.id AND \
				Gametypes.type = '%GAMETYPE%' AND \
				Gametypes.category Like '%GAMECAT%' AND \
				Players.siteId = %SITEID% AND \
				Players.name Like '%SCREENNAME%'",
	// stat_group
	fpdb_group_advanced
	},
#endif
		...
		// FPDB query to get vpip 
	{
		...
		// query
		"SELECT	sum(street0VPI)*1.0/sum(street0VPIChance)*1.0 as result \
		 FROM	HandsPlayers, Hands, Players, Gametypes \
		 WHERE	HandsPlayers.playerId = Players.id AND \
				HandsPlayers.handId = Hands.id AND \
				Hands.gametypeId = Gametypes.id AND \
				Gametypes.type = '%GAMETYPE%' AND \
	...
	// FPDB query to get pre-flop raise first in
	{
		...
		// query
		"SELECT	sum(raisedFirstIn)*1.0/sum(raiseFirstInChance)*1.0 as result \
		 FROM	HandsPlayers, Hands, Players, Gametypes \
		 WHERE	HandsPlayers.playerId = Players.id AND \
				HandsPlayers.handId = Hands.id AND \
				Hands.gametypeId = Gametypes.id AND \
...
#if FPDB_QUERY_SUPPORT__RFI_BTN_6MAX
	// FPDB query to get pre-flop raise first in
	{
		// name
		"rfi_btn_6max",
		// description_for_editor
		"FPDB pre-flop raise first in - Position BTN _6max",
		// query
...
#if FPDB_QUERY_SUPPORT__RFI_CO_6MAX
	// FPDB query to get pre-flop raise first in
	{
		// name
		"rfi_co_6max",
		// description_for_editor
		"FPDB pre-flop raise first in - Position CO _6max",
		// query
		"SELECT(case when ActionOpportunities = 0 then - 1 \
			 else cast(ActionCount as real) / ActionOpportunities \
			      end) as result \
...
#if FPDB_QUERY_SUPPORT__RFI_MP_6MAX
	// FPDB query to get pre-flop raise first in
	{
		// name
		"rfi_mp_6max",
		// description_for_editor
		"FPDB pre-flop raise first in - Position MP _6max",
		// query
		"SELECT(case when ActionOpportunities = 0 then - 1 \
			 else cast(ActionCount as real) / ActionOpportunities \
			      end) as result \
...
#if FPDB_QUERY_SUPPORT__RFI_EP_6MAX
	// FPDB query to get pre-flop raise first in
	{
		// name
		"rfi_ep_6max",
		// description_for_editor
		"FPDB pre-flop raise first in - Position EP _6max",
		// query
		"SELECT(case when ActionOpportunities = 0 then - 1 \
			 else cast(ActionCount as real) / ActionOpportunities \
			      end) as result \
...
#if FPDB_QUERY_SUPPORT__RFI_SB_6MAX
	// FPDB query to get pre-flop raise first in
	{
		// name
		"rfi_sb_6max",
		// description_for_editor
		"FPDB pre-flop raise first in - Position SB _6max",
		// query
		"SELECT(case when ActionOpportunities = 0 then - 1 \
			 else cast(ActionCount as real) / ActionOpportunities \
			      end) as result \
...
#if FPDB_QUERY_SUPPORT__RFI_BTN_9MAX
	// FPDB query to get pre-flop raise first in
	{
		// name
		"rfi_btn_9max",
		// description_for_editor
		"FPDB pre-flop raise first in - Position BTN _9max",
		// query
		"SELECT(case when ActionOpportunities = 0 then - 1 \
			 else cast(ActionCount as real) / ActionOpportunities \
			      end) as result \
...
#if FPDB_QUERY_SUPPORT__RFI_CO_9MAX
	// FPDB query to get pre-flop raise first in
	{
		// name
		"rfi_co_9max",
		// description_for_editor
		"FPDB pre-flop raise first in - Position CO _9max",
		// query
		"SELECT(case when ActionOpportunities = 0 then - 1 \
			 else cast(ActionCount as real) / ActionOpportunities \
			      end) as result \
...
#if FPDB_QUERY_SUPPORT__RFI_MP2_9MAX
	// FPDB query to get pre-flop raise first in
	{
		// name
		"rfi_mp2_9max",
		// description_for_editor
		"FPDB pre-flop raise first in - Position MP2 _9max",
		// query
		"SELECT(case when ActionOpportunities = 0 then - 1 \
			 else cast(ActionCount as real) / ActionOpportunities \
			      end) as result \
...
#if FPDB_QUERY_SUPPORT__RFI_MP1_9MAX
	// FPDB query to get pre-flop raise first in
	{
		// name
		"rfi_mp1_9max",
		// description_for_editor
		"FPDB pre-flop raise first in - Position MP1 _9max",
		// query
		"SELECT(case when ActionOpportunities = 0 then - 1 \
			 else cast(ActionCount as real) / ActionOpportunities \
			      end) as result \
...
#if FPDB_QUERY_SUPPORT__RFI_MP_9MAX
	// FPDB query to get pre-flop raise first in
	{
		// name
		"rfi_mp_9max",
		// description_for_editor
		"FPDB pre-flop raise first in - Position MP _9max",
		// query
		"SELECT(case when ActionOpportunities = 0 then - 1 \
			 else cast(ActionCount as real) / ActionOpportunities \
			      end) as result \
...
#if FPDB_QUERY_SUPPORT__RFI_EP1_9MAX
	// FPDB query to get pre-flop raise first in
	{
		// name
		"rfi_ep1_9max",
		// description_for_editor
		"FPDB pre-flop raise first in - Position EP1 _9max",
		// query
		"SELECT(case when ActionOpportunities = 0 then - 1 \
			 else cast(ActionCount as real) / ActionOpportunities \
			      end) as result \
...
#if FPDB_QUERY_SUPPORT__RFI_EP_9MAX
	// FPDB query to get pre-flop raise first in
	{
		// name
		"rfi_ep_9max",
		// description_for_editor
		"FPDB pre-flop raise first in - Position EP _9max",
		// query
		"SELECT(case when ActionOpportunities = 0 then - 1 \
			 else cast(ActionCount as real) / ActionOpportunities \
			      end) as result \
...
#if FPDB_QUERY_SUPPORT__RFI_SB_9MAX
	// FPDB query to get pre-flop raise first in
	{
		// name
		"rfi_sb_9max",
		// description_for_editor
		"FPDB pre-flop raise first in - Position SB _9max",
		// query
		"SELECT(case when ActionOpportunities = 0 then - 1 \
			 else cast(ActionCount as real) / ActionOpportunities \
			      end) as result \
...
#if FPDB_QUERY_SUPPORT__STEAL_ATTEMFPDB
	// FPDB query to get attemfpdbed to steal 
	{
		// name
		"steal_attemfpdb",
		// description_for_editor
		"FPDB attemfpdb to steal blinds",
		// query
		"SELECT (case when ActionOpportunities = 0 then -1 \
				 else cast(ActionCount as real) / ActionOpportunities \
				 end) as result \
