...
void CReplayFrame::CreateBitMapFile() {
  ...
  CString szFile = ReplayBitmapFilename(p_sessioncounter->session_id(), _next_replay_frame);
  HBITMAP hBMP = p_scraper->entire_window_cur();
  // Saves the hBitmap as a bitmap.
  SaveHBITMAPToFile(hBMP, szFile);
  /*
  ...
  // Create the .BMP file.
  hf = CreateFile((LPSTR)(LPCTSTR)szFile, GENERIC_READ | GENERIC_WRITE, (DWORD)0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, (HANDLE)NULL);
