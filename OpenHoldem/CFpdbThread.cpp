...
CFpdbThread::CFpdbThread()
{
	...
	pUpdateFpdbStat(UpdateFpdbStat);
}
...
double UpdateFpdbStat(int m_chr, int stat)
{
	UINT		res = NULL;
	...
	if (!p_fpdb_thread->_connected || pSQLite->GetDbStatus() != SQLITE_OK)
		return kUndefined;
	...
	if (p_fpdb_thread->CheckIfNameExistsInDB(m_chr) == false)
	{
		...
		write_log(Preferences()->debug_pokertracker(), "[Fpdb] UpdateFpdbStat for chair[%d] had been skipped. Reason: [CheckName failed]\n", m_chr);
		return kUndefined;
	}
  ...
  CString query = FPDB_DLL_GetQuery(stat,
	  ...
	  siteid, _fpdb_player_data[m_chr].fpdb_name, p_tablemap->nchairs(),
	  ...
	  p_engine_container->symbol_engine_tablelimits()->ante(),
	  p_table_state->_s_limit_info.is_final_table(), fpdb_nb_hands, fpdb_time_period);
	...
	catch (_com_error &e)
	{
		...
		write_log(Preferences()->debug_fpdb(), _T("\tDescription = %s\n"), (LPCTSTR) bstrDescription);
	...
	else
	{
		...
		{
			query_result->Next();
			if (query_result->ColomnData(0) != NULL)
