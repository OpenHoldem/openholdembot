...
t_QueryDefinition query_definitions[k_number_of_pokertracker_stats] =
{
	...
	// PT4 
	{
		...
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(CASE WHEN S.cnt_p_raise > 0 THEN 1 ELSE 0 END) \
						 as ActionCount, \
				(CASE WHEN LA.action LIKE '__%' OR (LA.action LIKE '_' AND (S.amt_before > (TL.amt_bb + S.amt_ante)) AND (S.amt_p_raise_facing < (S.amt_before - (S.amt_blind + S.amt_ante))) AND (S.flg_p_open_opp OR S.cnt_p_face_limpers > 0 OR S.flg_p_3bet_opp OR S.flg_p_4bet_opp) ) THEN 1 ELSE 0 END) \
						 as ActionOpportunities \
				FROM	player as P, %TYPE%_hand_player_statistics as S, lookup_actions as LA %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
						%TOURNEYTABLETYPE% \
						%POSITIONSTATEMENT% \
	...
	// PT4 query to get vpip 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
			FROM   (SELECT   (case when flg_vpip then 1 else 0 end) \
					as ActionCount, \
					((CASE WHEN S.id_hand > 0 THEN 1 ELSE 0 END) -(CASE WHEN LA_P.action = '' THEN 1 ELSE 0 END)) \
					as ActionOpportunities \
            FROM	player as P, %TYPE%_hand_player_statistics as S, lookup_actions as LA_P %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
            WHERE   S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
					%LIMITSTATEMENT% \
					%TOURNEYTABLETYPE% \
					%POSITIONSTATEMENT% \
	...
	// PT4 query to get number of hands in the database 
	{
		...
		"SELECT count(*) as result \
		 FROM   player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
		 WHERE  S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
				%LIMITSTATEMENT% \
				%TOURNEYTABLETYPE% \
				%POSITIONSTATEMENT% \
	...
	// PT4 query to get pre-flop raise first in
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when(s.flg_p_first_raise AND s.flg_p_open_opp) then  1 else  0 end) \
						 as ActionCount, \
						(case when(s.flg_p_open_opp) then  1 else  0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 query to get Went to SD 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when flg_showdown then 1 else 0 end) \
						 as ActionCount, \
	...
	// PT4 query to get Won $ At SD 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	 (case when (s.flg_showdown AND s.flg_won_hand) then  1 else  0 end) \
						 as ActionCount, \
						(case when(s.flg_showdown) then  1 else  0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 query to get attempted to steal 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when flg_steal_att then 1 else 0 end) \
						 as ActionCount, \
						(case when flg_steal_opp then 1 else 0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 query to get Folded Big Blind to steal 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when flg_bb_steal_fold then 1 else 0 end) \
						 as ActionCount, \
						(case when flg_blind_def_opp AND flg_blind_b then 1 else 0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 query to get Folded Small Blind to steal 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when flg_sb_steal_fold then 1 else 0 end) \
						 as ActionCount, \
						(case when flg_blind_def_opp AND flg_blind_s then 1 else 0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 query to get 3B vs. steal 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when (flg_blind_def_opp AND flg_p_3bet) then 1 else 0 end) \
						 as ActionCount, \
						(case when (flg_blind_def_opp AND flg_p_3bet_opp) then  1 else  0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 query to get BB 3B vs. steal 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM (SELECT   (case when (flg_blind_b AND flg_blind_def_opp AND flg_p_3bet) then 1 else 0 end) \
				  as ActionCount, \
				  (case when (flg_blind_b AND flg_blind_def_opp AND flg_p_3bet_opp) then 1 else 0 end) \
				  as ActionOpportunities \
				FROM player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 query to get SB 3B vs. steal 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		FROM (SELECT (case when (flg_blind_s AND flg_blind_def_opp AND flg_p_3bet) then 1 else 0 end) \
				  as ActionCount, \
				  (case when (flg_blind_s AND flg_blind_def_opp AND flg_p_3bet_opp) then 1 else 0 end) \
				  as ActionOpportunities \
				FROM   player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				WHERE   S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 query to get pre-flop called raise 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when ((s.amt_p_2bet_facing > 0 AND NOT(s.flg_p_limp) AND NOT s.flg_p_3bet_def_opp AND NOT flg_p_4bet_def_opp AND cnt_p_call>0) or \
						(s.enum_p_3bet_action='C') or (s.enum_p_4bet_action='C')) then 1 else 0 end) \
						 as ActionCount, \
						(case when s.flg_p_3bet_def_opp or s.flg_p_4bet_def_opp or s.amt_p_2bet_facing > 0 then 1 else 0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4  query to get PREFLOP 3 bet 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when flg_p_3bet then 1 else 0 end) \
						 as ActionCount, \
	...
	// PT4  query to get PREFLOP fold to 3 bet 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when(s.enum_p_3bet_action='F') then  1 else  0 end) \
						 as ActionCount, \
						(case when(s.flg_p_3bet_def_opp) then  1 else  0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4  query to get PREFLOP fold to 3 bet 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when(s.enum_p_3bet_action='F' AND s.flg_p_first_raise) then  1 else  0 end) \
						 as ActionCount, \
						(case when(s.flg_p_3bet_def_opp AND s.flg_p_first_raise) then  1 else  0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 query to get Fold to Resteal 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when(s.flg_steal_att AND s.flg_p_3bet_def_opp AND s.enum_p_3bet_action='F') then  1 else  0 end) \
						 as ActionCount, \
						(case when(s.flg_steal_att AND s.flg_p_3bet_def_opp ) then  1 else  0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 query to get Preflop 4B 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when flg_p_4bet then 1 else 0 end) \
						 as ActionCount, \
	...
	// PT4 query to get Preflop fold to 4B 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when (enum_p_4bet_action='F') then 1 else 0 end) \
						 as ActionCount, \
						(case when flg_p_4bet_def_opp then 1 else 0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(CASE WHEN S.flg_p_squeeze THEN 1 ELSE 0 END) \
						 as ActionCount, \
				(CASE WHEN S.flg_p_squeeze_opp THEN 1 ELSE 0 END) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 query to get flop aggression factor 
	{
		...
		// query
		"SELECT (cast(sum(AggressionCount)as real) / sum(CallsCount)) as result \
		 FROM	(SELECT	(S.cnt_f_raise) + (case when S.flg_f_bet then 1 else 0 end) \
						 as AggressionCount, \
						(S.cnt_f_call) \
						 as CallsCount \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
	...
	// PT4 query to get turn aggression factor 
	{
		...
		// query
		"SELECT (cast(sum(AggressionCount)as real) / sum(CallsCount)) as result \
		 FROM	(SELECT	(S.cnt_t_raise) + (case when S.flg_t_bet then 1 else 0 end) \
						 as AggressionCount, \
						(S.cnt_t_call) \
						 as CallsCount \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
	...
	// PT4 query to get river aggression factor 
	{
		...
		// query
		"SELECT (cast(sum(AggressionCount)as real) / sum(CallsCount)) as result \
		 FROM	(SELECT	(S.cnt_r_raise) + (case when S.flg_r_bet then 1 else 0 end) \
						 as AggressionCount, \
						(S.cnt_r_call) \
						 as CallsCount \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
	...
	// PT4 query to get total aggression factor without preflop 
	{
		...
		// query
		"SELECT (cast(sum(AggressionCount)as real) / sum(CallsCount)) as result \
		 FROM (SELECT	 (cnt_f_raise) + (case when flg_f_bet then 1 else 0 end) \
					   + (cnt_t_raise) + (case when flg_t_bet then 1 else 0 end) \
                       + (cnt_r_raise) + (case when flg_r_bet then 1 else 0 end) \
						  as AggressionCount, \
						 (cnt_f_call) \
                       + (cnt_t_call) \
                       + (cnt_r_call) \
	...
	// PT4 query to get flop aggression frequency 
	{
		...
		// query
		"SELECT (cast(sum(AggressionCount)as real) / sum(AggressionCount + PassiveCount)) as result \
		 FROM (SELECT	(cnt_f_raise) + (case when flg_f_bet then 1 else 0 end) \
						 as AggressionCount, \
						(cnt_f_call) + (case when flg_f_fold then 1 else 0 end) \
						 as PassiveCount \
			   FROM   player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
			   WHERE  S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 query to get turn aggression frequency 
	{
		...
		// query
		"SELECT (cast(sum(AggressionCount)as real) / sum(AggressionCount + PassiveCount)) as result \
		 FROM (SELECT	(cnt_t_raise) + (case when flg_t_bet then 1 else 0 end) \
						 as AggressionCount, \
						(cnt_t_call) + (case when flg_t_fold then 1 else 0 end) \
						 as PassiveCount \
			   FROM   player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
			   WHERE  S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 query to get river aggression frequency 
	{
		...
		// query
		"SELECT (cast(sum(AggressionCount)as real) / sum(AggressionCount + PassiveCount)) as result \
		 FROM (SELECT	(cnt_r_raise) + (case when flg_r_bet then 1 else 0 end) \
						 as AggressionCount, \
						(cnt_r_call) + (case when flg_r_fold then 1 else 0 end) \
						 as PassiveCount \
			   FROM   player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
			   WHERE  S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 query to get turns seen 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when s.flg_t_saw then 1 else 0 end) \
						 as ActionCount, \
						(case when (s.flg_f_saw) then 1 else 0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 query to get rivers seen 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when s.flg_r_saw then 1 else 0 end) \
						 as ActionCount, \
						(case when (s.flg_t_saw) then 1 else 0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4  query to get cbet on FLOP 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when flg_f_cbet then 1 else 0 end) \
						 as ActionCount, \
						(case when flg_f_cbet_opp then 1 else 0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4  query to get FLOP fold to cbet 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when enum_f_cbet_action = 'F' then 1 else 0 end) \
						 as ActionCount, \
						(case when flg_f_cbet_def_opp then 1 else 0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4  query to get raise cbet on FLOP 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when S.enum_f_cbet_action = 'R' then 1 else 0 end) \
						 as ActionCount, \
						(case when S.flg_f_cbet_def_opp = 't' then 1 else 0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4  query to get cbet on TURN 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when S.flg_t_cbet = 't' then 1 else 0 end) \
						 as ActionCount, \
						(case when flg_t_cbet_opp then 1 else 0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4  query to get TURN fold to cbet 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when enum_t_cbet_action = 'F' then 1 else 0 end) \
						 as ActionCount, \
						(case when flg_t_cbet_def_opp then 1 else 0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4  query to get RIVER fold to cbet 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when enum_r_cbet_action = 'F' then 1 else 0 end) \
						 as ActionCount, \
						(case when flg_r_cbet_def_opp then 1 else 0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4  query to get total fold to 3 bet without preflop
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when enum_f_3bet_action = 'F' or enum_t_3bet_action ='F' or enum_r_3bet_action = 'F' then 1 else 0 end) \
						 as ActionCount, \
						(case when flg_f_3bet_def_opp or flg_t_3bet_def_opp or flg_r_3bet_def_opp then 1 else 0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4  query to get FLOP fold to 3 bet 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when enum_f_3bet_action = 'F' then 1 else 0 end) \
						 as ActionCount, \
	...
	// PT4  query to get flop check-raise 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when (flg_f_check_raise) then 1 else 0 end) \
						 as ActionCount, \
						(case when (flg_f_check=TRUE AND (cnt_f_raise>0 OR cnt_f_call>0 OR flg_f_fold)) then 1 else 0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4  query to get FLOP donk 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when (S.flg_p_face_raise AND NOT(S.flg_p_3bet OR S.flg_p_4bet) \
						...
						(HSum.cnt_players = 2 and S.flg_blind_b))) then 1 else 0 end) \
						 as ActionCount, \
						(case when (S.flg_p_face_raise AND NOT(S.flg_p_3bet OR S.flg_p_4bet) \
						AND (S.flg_f_open_opp) AND NOT(S.flg_f_cbet_opp) AND \
						((HSum.cnt_players > 2 and S.val_p_raise_aggressor_pos < S.position) or \
						(HSum.cnt_players = 2 and S.flg_blind_b))) then 1 else 0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_summary as HSum, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
						%TOURNEYTABLETYPE% \
						%POSITIONSTATEMENT% \
	...
	// PT4  query to get flop float 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
				 FROM	(SELECT	(case when ((cnt_p_call>0) AND S.flg_p_face_raise AND S.flg_f_bet \
								...
								AND ((HSum.cnt_players > 2 AND S.val_p_raise_aggressor_pos > S.position) \
								 ...
								 as ActionCount, \
								(case when ((cnt_p_call>0) AND S.flg_p_face_raise AND S.flg_f_open_opp \
								AND char_length(HSum.str_aggressors_p)=2 \
								AND((HSum.cnt_players>2 AND S.val_p_raise_aggressor_pos > S.position) \
									OR(HSum.cnt_players=2 AND S.flg_f_has_position))) then 1 else 0 end) \
								 as ActionOpportunities \
	...
	// PT4  query to get TURN fold to 3 bet 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when enum_t_3bet_action = 'F' then 1 else 0 end) \
						 as ActionCount, \
	...
	// PT4  query to get TURN Check-Raise 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when (flg_t_check_raise) then 1 else 0 end) \
						 as ActionCount, \
						(case when (flg_t_check=TRUE AND (cnt_t_raise>0 OR cnt_t_call>0 OR flg_t_fold)) then 1 else 0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4  query to get TURN Check-Call 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when (S.flg_t_check AND S.cnt_t_call>0) then 1 else 0 end) \
						 as ActionCount, \
						(case when (S.flg_t_check= 't' AND (S.cnt_t_raise>0 OR S.cnt_t_call>0 OR S.flg_t_fold= 't')) then 1 else 0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4  query to get RIVER fold to 3 bet 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when enum_r_3bet_action = 'F' then 1 else 0 end) \
						 as ActionCount, \
						(case when flg_r_3bet_def_opp then 1 else 0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4  query to get River bet 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(case when flg_r_bet then 1 else 0 end) \
						 as ActionCount, \
						(case when (flg_r_bet or flg_r_check) then 1 else 0 end) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		FROM (SELECT (CASE WHEN(S.flg_p_3bet_def_opp OR S.flg_p_4bet_def_opp) AND NOT(S.flg_p_3bet OR S.flg_p_4bet) AND S.flg_f_bet AND NOT(S.flg_f_cbet_opp) AND((%TYPE%_hand_summary.cnt_players > 2 AND S.val_p_raise_aggressor_pos < S.position) or (%TYPE%_hand_summary.cnt_players = 2 and S.flg_blind_b)) THEN 1 ELSE 0 END) \
				 as ActionCount, \
				 (CASE WHEN(S.flg_p_3bet_def_opp OR S.flg_p_4bet_def_opp) AND NOT(S.flg_p_3bet OR S.flg_p_4bet) AND(S.flg_f_open_opp) AND NOT(S.flg_f_cbet_opp) AND((%TYPE%_hand_summary.cnt_players > 2 AND S.val_p_raise_aggressor_pos < S.position) or (%TYPE%_hand_summary.cnt_players = 2 AND S.flg_blind_b)) THEN 1 ELSE 0 END) \
				 as ActionOpportunities \
			FROM	player as P, %TYPE%_hand_player_statistics as S, %TYPE%_hand_summary %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
			WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
					%LIMITSTATEMENT% \
					%TOURNEYTABLETYPE% \
					%POSITIONSTATEMENT% \
					S.id_player = P.id_player AND \
					%TYPE%_hand_summary.id_hand = S.id_hand AND \
					S.id_gametype = %GAMETYPE% AND \
					P.id_site = %SITEID% AND \
					P.player_name LIKE '%SCREENNAME%' \
				ORDER BY S.date_played DESC \
	...
	// PT4 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(CASE WHEN (S.flg_p_3bet_def_opp OR S.flg_p_4bet_def_opp) AND S.enum_f_cbet_action='F' THEN 1 ELSE 0 END) \
						 as ActionCount, \
				(CASE WHEN (S.flg_p_3bet_def_opp OR S.flg_p_4bet_def_opp) AND S.flg_f_cbet_def_opp THEN 1 ELSE 0 END) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(CASE WHEN S.flg_f_cbet AND (S.flg_p_3bet or S.flg_p_4bet) THEN 1 ELSE 0 END) \
						 as ActionCount, \
				(CASE WHEN S.flg_f_cbet_opp AND (S.flg_p_3bet or S.flg_p_4bet) THEN 1 ELSE 0 END) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(CASE WHEN S.flg_f_check_raise AND S.flg_p_3bet THEN 1 ELSE 0 END) \
						 as ActionCount, \
				(CASE WHEN S.flg_f_check AND S.flg_p_3bet AND (S.cnt_f_raise > 0 OR S.cnt_f_call > 0 OR S.flg_f_fold) THEN 1 ELSE 0 END) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(CASE WHEN S.flg_f_check_raise AND (S.flg_p_3bet or S.flg_p_4bet) THEN 1 ELSE 0 END) \
						 as ActionCount, \
				(CASE WHEN S.flg_f_check AND (S.flg_p_3bet or S.flg_p_4bet) AND (S.cnt_f_raise > 0 OR S.cnt_f_call > 0 OR S.flg_f_fold) THEN 1 ELSE 0 END) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(CASE WHEN substring(LA.action from 2 for 1) = 'F' AND S.flg_f_bet AND S.flg_f_face_raise AND ((%TYPE%_hand_summary.cnt_players > 2 AND substring(%TYPE%_hand_summary.str_aggressors_f from 2 for 1)::int > S.position) OR (%TYPE%_hand_summary.cnt_players = 2 AND S.flg_blind_s)) THEN 1 ELSE 0 END) \
						 as ActionCount, \
				(CASE WHEN S.flg_f_bet AND S.flg_f_face_raise AND (( %TYPE%_hand_summary.cnt_players > 2 AND substring(%TYPE%_hand_summary.str_aggressors_f from 2 for 1)::int > S.position) OR (%TYPE%_hand_summary.cnt_players = 2 AND S.flg_blind_s)) THEN 1 ELSE 0 END) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S, %TYPE%_hand_summary, lookup_actions as LA %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
						%TOURNEYTABLETYPE% \
						%POSITIONSTATEMENT% \
	...
	// PT4 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(CASE WHEN substring(LA.action from 2 for 1) = 'F' AND S.flg_f_bet AND S.flg_f_face_raise AND ((%TYPE%_hand_summary.cnt_players > 2 AND substring(%TYPE%_hand_summary.str_aggressors_f from 2 for 1)::int > S.position) OR (%TYPE%_hand_summary.cnt_players = 2 AND S.flg_blind_s)) AND (S.flg_p_3bet or S.flg_p_4bet) THEN 1 ELSE 0 END) \
						 as ActionCount, \
				(CASE WHEN S.flg_f_bet AND S.flg_f_face_raise AND ((%TYPE%_hand_summary.cnt_players > 2 AND substring(%TYPE%_hand_summary.str_aggressors_f from 2 for 1)::int > S.position) OR (%TYPE%_hand_summary.cnt_players = 2 AND S.flg_blind_s)) AND (S.flg_p_3bet or S.flg_p_4bet) THEN 1 ELSE 0 END) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S, %TYPE%_hand_summary, lookup_actions as LA %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(CASE WHEN (LA.action = 'C' OR LA.action = 'CC') AND S.flg_p_face_raise AND S.flg_f_bet AND char_length(%TYPE%_hand_summary.str_aggressors_p) = 2 AND ((%TYPE%_hand_summary.cnt_players > 2 AND substring(%TYPE%_hand_summary.str_aggressors_p from 2 for 1)::int > S.position) OR (%TYPE%_hand_summary.cnt_players = 2 AND S.flg_f_has_position)) THEN 1 ELSE 0 END) \
						 as ActionCount, \
				(CASE WHEN (LA.action = 'C' OR LA.action = 'CC') AND S.flg_p_face_raise AND S.flg_f_open_opp AND char_length(%TYPE%_hand_summary.str_aggressors_p) = 2 AND ((%TYPE%_hand_summary.cnt_players > 2 AND substring(%TYPE%_hand_summary.str_aggressors_p from 2 for 1)::int > S.position) OR (%TYPE%_hand_summary.cnt_players = 2 AND S.flg_f_has_position)) THEN 1 ELSE 0 END) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S, %TYPE%_hand_summary, lookup_actions as LA %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(CASE WHEN S.flg_p_first_raise and LA.action = 'RC' and S.flg_f_has_position and S.flg_f_bet THEN 1 ELSE 0 END) \
						 as ActionCount, \
				(CASE WHEN S.flg_p_first_raise and LA.action = 'RC' and S.flg_f_has_position and S.flg_f_open_opp THEN 1 ELSE 0 END) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S, lookup_actions as LA %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(CASE WHEN not(S.flg_p_face_raise) AND LA_P.action LIKE '%R' and S.flg_f_open_opp AND S.amt_f_bet_facing > 0 AND substring(LA_F.action from 1 for 2) = 'XC' THEN 1 ELSE 0 END) \
						 as ActionCount, \
				(CASE WHEN not(S.flg_p_face_raise) AND LA_P.action LIKE '%R' AND S.flg_f_open_opp AND S.flg_f_check AND S.amt_f_bet_facing > 0 THEN 1 ELSE 0 END) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S, lookup_actions as LA_P, lookup_actions as LA_F %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
						%TOURNEYTABLETYPE% \
						%POSITIONSTATEMENT% \
	...
	// PT4 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(CASE WHEN not(S.flg_p_face_raise) AND LA_P.action LIKE '%R' and S.flg_f_open_opp AND S.amt_f_bet_facing > 0 AND substring(LA_F.action from 1 for 2) = 'XC' AND S.flg_p_3bet THEN 1 ELSE 0 END) \
						 as ActionCount, \
				(CASE WHEN not(S.flg_p_face_raise) AND LA_P.action LIKE '%R' AND S.flg_f_open_opp AND S.flg_f_check AND S.amt_f_bet_facing > 0 AND S.flg_p_3bet THEN 1 ELSE 0 END) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S, lookup_actions as LA_P, lookup_actions as LA_F %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(CASE WHEN not(S.flg_p_face_raise) AND LA_P.action LIKE '%R' and S.flg_f_open_opp AND S.amt_f_bet_facing > 0 AND substring(LA_F.action from 1 for 2) = 'XF' AND S.flg_p_3bet THEN 1 ELSE 0 END) \
						 as ActionCount, \
				(CASE WHEN not(S.flg_p_face_raise) AND LA_P.action LIKE '%R' AND S.flg_f_open_opp AND S.flg_f_check AND S.amt_f_bet_facing > 0 AND S.flg_p_3bet THEN 1 ELSE 0 END) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S, lookup_actions as LA_P, lookup_actions as LA_F %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
	...
	// PT4 
	{
		...
		// query
		"SELECT (cast(sum(ActionCount)as real) / sum(ActionOpportunities)) as result \
		 FROM	(SELECT	(CASE WHEN not(S.flg_p_face_raise) AND LA_P.action LIKE '%R' and S.flg_f_open_opp AND S.amt_f_bet_facing > 0 AND substring(LA_F.action from 1 for 2) = 'XF' AND (S.flg_p_3bet or S.flg_p_4bet) THEN 1 ELSE 0 END) \
						 as ActionCount, \
				(CASE WHEN not(S.flg_p_face_raise) AND LA_P.action LIKE '%R' AND S.flg_f_open_opp AND S.flg_f_check AND S.amt_f_bet_facing > 0 AND (S.flg_p_3bet or S.flg_p_4bet) THEN 1 ELSE 0 END) \
						 as ActionOpportunities \
				 FROM	player as P, %TYPE%_hand_player_statistics as S, lookup_actions as LA_P, lookup_actions as LA_F %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
	...
	// PT4 
	{
		...
		FROM	player as P, %TYPE%_hand_player_statistics as S,  %TYPE%_hand_summary as HSum %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
						%TOURNEYTABLETYPE% \
						%POSITIONSTATEMENT% \
	...
	// PT4 
	{
		...
		// query
		"SELECT sum(CASE WHEN S.flg_f_bet AND S.flg_f_face_raise AND (( %TYPE%_hand_summary.cnt_players > 2 AND substring( %TYPE%_hand_summary.str_aggressors_f from 2 for 1)::int > S.position) OR ( %TYPE%_hand_summary.cnt_players = 2 AND S.flg_blind_s)) THEN 1 ELSE 0 END) as result \
		FROM	player as P, %TYPE%_hand_player_statistics as S, %TYPE%_hand_summary %TYPELIMIT% %TOURNEYTYPESELECT% %PLAYERPOSITION% \
				 WHERE	S.date_played > NOW() - interval '%TIMEPERIOD%' AND \
						%LIMITSTATEMENT% \
						%TOURNEYTABLETYPE% \
